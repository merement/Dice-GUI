...
from dice_gui.widgets.trace_window import TraceWindow, TracePlotWidget, TIME_WINDOW
...
def test_trace_window_bounds_calculation(qapp):
    times = np.arange(150, dtype=float) * 0.1
    spins = np.ones((150, 3), dtype=np.int8)
    x_values = np.zeros((150, 3), dtype=float)
    ...
    tw = TraceWindow(node_index=1, point_id="test_id", simulation_data=data, parent_window=window)
    
    assert tw.node_index == 1
    assert tw.point_id == "test_id"
    assert tw.windowTitle() == "Point # 2  Id: test_id"
    assert tw.bound_left == 0
    assert tw.bound_right == TIME_WINDOW  # 50
    assert tw.current_frame == 0

    # 2. Update frame within bounds (e.g. 30): bounds should not change
    tw.set_frame_index(30)
    assert tw.bound_left == 0
    assert tw.bound_right == TIME_WINDOW
    assert tw.current_frame == 30

    # 3. Update frame out of bounds (e.g. 60): bounds should shift
    tw.set_frame_index(60)
    assert tw.bound_left == 60 - TIME_WINDOW  # 10
    assert tw.bound_right == 60 + TIME_WINDOW  # 110
    assert tw.current_frame == 60

    # 4. Check boundaries limits at the end of frames (e.g. 145)
    tw.set_frame_index(145)
    assert tw.bound_left == 145 - TIME_WINDOW  # 95
    assert tw.bound_right == 149  # min(149, 195)
    assert tw.current_frame == 145

    tw.close()


def test_trace_plot_widget_colors(qapp):
    times = np.arange(10, dtype=float)
    spins = np.array([[1, -1, 1]] * 10, dtype=np.int8)
    x_values = np.zeros((10, 3), dtype=float)
    data = TimeSeriesData(times=times, spins=spins, x_values=x_values)

    plot_widget = TracePlotWidget(simulation_data=data, node_index=0)
...
def test_main_window_trace_handling(qapp):
    ...
    node_indices = {tw.node_index for tw in window.trace_windows}
    ...
    # 4. Loading a new simulation should close all trace windows
    window.on_simulation_loaded(loaded_sim)
    assert len(window.trace_windows) == 0


def test_trace_plot_widget_x_bounds(qapp):
